...
数据读入：通过Dataset+DataLoader的方式完成的，Dataset定义好数据的格式和数据变换形式，DataLoader用iterative的方式不断读入批次数据。
...
```
    ...
    tensor([[ 0.0051,  0.0357, -0.0931, -0.1537, -0.0860, -0.0900,  0.1357,  0.1173,
             -0.0081,  0.0041],
            [ 0.1329,  0.0626, -0.0346, -0.2598, -0.0645,  0.0479,  0.2330, -0.0539,
              0.1223, -0.1551]], grad_fn=<AddmmBackward0>)



### 4.2 神经网络中常见的层

- 神经网络中各式各样的层，例如全连接层、卷积层、池化层与循环层等。
...
```
    ...
    tensor([[ 0.0736,  0.0843, -0.0592, -0.0298, -0.0024,  0.0397, -0.0746,  0.0241,
             -0.0546, -0.0112]], grad_fn=<AddmmBackward0>)
...
```
    ...
    tensor([ 0.0219,  0.0555, -0.0171,  0.1906, -0.0044, -0.0139])
...
```
    ...
    tensor([[[[ 0.0269,  0.2714, -0.3254],
              [ 0.0917, -0.1546, -0.3217],
              [ 0.1759, -0.0209,  0.0452]]],
    
    
            [[[ 0.1574, -0.0946, -0.3180],
              [-0.2436, -0.1966,  0.1001],
              [ 0.2418,  0.0102, -0.0198]]],
    
    
            [[[-0.1663, -0.2928, -0.0526],
              [-0.1092,  0.0836, -0.1970],
              [ 0.1205,  0.1892, -0.1585]]]])
    tensor([[ 0.0811, -0.2933,  0.0161,  0.1222, -0.0815, -0.2118,  0.1733,  0.0393,
             -0.0059, -0.2448]])
...
```
    ...
    tensor([[[[ 0.0767, -0.4766, -0.0849],
              [ 0.5576,  0.4050,  0.0625],
              [-0.0743, -0.2180,  0.0043]]],
    
    
            [[[ 0.5506, -0.1580, -0.4898],
              [ 0.2184, -0.0991,  0.2266],
              [ 0.5418, -0.1655, -0.1854]]],
    
    
            [[[-0.0259,  0.1309, -0.4158],
              [ 0.7980,  0.1827, -0.2782],
              [-0.0105, -0.4473, -1.0750]]]])
...
```
    ...
    tensor([[[[-0.1875, -0.2133, -0.2460],
              [-0.0259,  0.2476, -0.3008],
              [ 0.0018,  0.1611,  0.0716]]]])
    -------初始化-------
    tensor([[[[0., 0., 0.],
              [0., 0., 0.],
              [0., 0., 0.]]]])
